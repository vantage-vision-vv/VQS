...
class GGNN():
	...
	def Rnn():
		...
		for i in range(no_of_roles):
			X_total = tf.add(
			    tf.matmul(self.weights['message'], self.role_nodes[i]), X_total)
	...
	def model(self, v, r, verb_encoding):
		...
		self.verb_node = tf.nn.relu(
		    tf.matmul(self.transform['verb'], self.placeholder['verb']))
		for i in range(no_of_roles):
			self.role_nodes[i] = tf.nn.relu(tf.multiply(tf.multiply(tf.matmul(self.transform['noun'], self.placeholder['verb']), tf.matmul(
			    self.embeddings['noun'], One_hot_encoder(no_of_roles, i))), tf.matmul(self.embeddings['noun'], verb_encoding)))
		for time in range(time_steps):  # Try scan
			Rnn()

	def initialize(self):
		self.placeholders['verb'] = tf.placeholders(
		    "float", [input_feature_dim])  # I x 1
		self.placeholders['noun'] = tf.placeholders(
		    "float", [no_of_rolesinput_feature_dim])  # R x I
		for i in range(no_of_roles):
			var_name = hidden_state_ + str(i)
			self.role_nodes[i] = tf.get_variable(
			    var_name, shape_name=[hidden_state_dim])  # H
		...
		self.embeddings['noun']=tf.get_variable("noun_embedding", shape=[
		                                        hidden_state_dim, num_nouns], initializer=tf.contrib.layers.xavier_initializer())  # H x N
		...
		self.biases['B_z']=tf.get_variable("B_z", shape=[
		                                   hidden_state_dim], initializer=tf.contrib.layers.xavier_initializer())  # H x 1
		self.biases['B_r']=tf.get_variable("B_r", shape=[
		                                   hidden_state_dim], initializer=tf.contrib.layers.xavier_initializer())  # H x 1
		self.biases['B_h']=tf.get_variable("B_h", shape=[
		                                   hidden_state_dim], initializer=tf.contrib.layers.xavier_initializer())  # H x 1
		self.biases['message']=tf.get_variable("message", shape=[
		                                       hidden_state_dim], initializer=tf.contrib.layers.xavier_initializer())  # H x 1
