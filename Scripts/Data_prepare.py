...
def extract_object_info(name,object_encoder):
	...
	object_type = [] 
	with open(name+".viratdata.objects.txt","r") as f:
		for line in f:
...
def driver():
	cnt = 0
	for name in files:
		with open(name+".viratdata.mapping.txt","r") as f:
			for line in f:
				data = line.strip().split(" ")
				ev_meta = data[:5]
				...
				np.savez(out_path+ev_meta[1]+"_"+str(cnt),np.array(objects),frame_data)
				cnt += 1
